...
test("CLI packs and inspects a fixture repository", async () => {
  ...
  try {
    ...
    const pack = spawnSync(process.execPath, ["dist/cli.js", "pack", fixture, "--output", output], { encoding: "utf8" });
    assert.equal(pack.status, 0, pack.stderr);
    assert.match(pack.stdout, /Packed/);

    const inspect = spawnSync(process.execPath, ["dist/cli.js", "inspect", output], { encoding: "utf8" });
